...
export const Avatar = ({ alt, src, size, ...rest }: { alt: string; src: string; size: number }) => (
  <img
    alt={alt}
    src={src}
    {...rest}
    sx={{
      ...
      boxShadow: `0 0 1px ${Math.round(size / 10)}px var(--box-shadow-color-dark), 0 0 1px ${Math.round(
        size / 5
      ...
      ':hover': {
        boxShadow: `0 0 1px ${Math.round(size / 3)}px var(--box-shadow-color-med), 0 0 1px ${Math.round(
          size / 1.5
